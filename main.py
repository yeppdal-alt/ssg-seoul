...
# ────────────────────────────────────────────────────────────
# 0-2. 매물보다 중개사가 많은 동네? (부동산 중개업 밀도 유머 코너)
# ────────────────────────────────────────────────────────────
st.header("🏘️ 매물보다 중개사가 더 많은 동네가 있다?")
st.caption(
    "※ 실제 매물 수 데이터는 없어서, 행정동 내 '고유 위경도 좌표 수(=건물 추정치)'를 매물 후보군으로 삼아 "
    "부동산 중개업소 수와 비교해본 유머용 지표입니다. 학술적 통계가 아니라 동네 드립이니 가볍게 봐주세요 😂"
)

realtor = df[df["상권업종소분류명"].str.contains("부동산 중개", na=False)]
building_proxy = (
    df.groupby(["시군구명", "행정동명"])[["경도", "위도"]]
    .apply(lambda x: x.drop_duplicates().shape[0])
    .reset_index(name="고유건물수(추정)")
)
realtor_cnt = realtor.groupby(["시군구명", "행정동명"]).size().reset_index(name="중개업소수")

density_df = realtor_cnt.merge(building_proxy, on=["시군구명", "행정동명"])
density_df["건물당 중개사(개)"] = (density_df["중개업소수"] / density_df["고유건물수(추정)"]).round(2)
density_df["동네"] = density_df["시군구명"] + " " + density_df["행정동명"]
density_df = density_df.sort_values("건물당 중개사(개)", ascending=False)

overload_n = (density_df["중개업소수"] >= density_df["고유건물수(추정)"]).sum()
top_area = density_df.iloc[0]
st.warning(
    f"🏆 서울에서 가장 '중개사 밀도'가 높은 동네는 **{top_area['동네']}**! "
    f"건물 추정치 {top_area['고유건물수(추정)']:.0f}개당 중개업소 {top_area['중개업소수']:.0f}개 "
    f"(건물 1개당 중개사 {top_area['건물당 중개사(개)']}개꼴). "
    f"이 지표로만 보면 실제로 '중개업소 수 ≥ 건물 추정치'인 동네가 서울에 **{overload_n}곳**이나 있어요."
)

top15 = density_df.head(15).sort_values("건물당 중개사(개)", ascending=True)
fig_realtor = px.bar(
    top15, x="건물당 중개사(개)", y="동네", orientation="h", text="건물당 중개사(개)",
    color="건물당 중개사(개)", color_continuous_scale="Purples",
    title="건물당 부동산 중개업소 밀도 TOP 15 (건물은 고유 좌표수로 추정)",
)
fig_realtor.update_traces(textposition="outside")
fig_realtor.update_layout(yaxis=dict(title=None), height=550, coloraxis_showscale=False)
st.plotly_chart(fig_realtor, use_container_width=True)

st.markdown("#### 동네별 중개업소 밀도 상세")
st.dataframe(
    density_df[["동네", "중개업소수", "고유건물수(추정)", "건물당 중개사(개)"]].reset_index(drop=True),
