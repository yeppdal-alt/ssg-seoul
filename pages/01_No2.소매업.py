...
# 기본 설정
# ────────────────────────────────────────────────────────────
st.set_page_config(page_title="소매업종 전문 분석", page_icon="🛍️", layout="wide")

DATA_PATH = "data_seoul.csv"   # app.py(또는 main.py)와 같은 폴더에 있는 데이터를 그대로 사용합니다.
...
df_all = load_data()
retail = df_all[df_all["상권업종대분류명"] == "소매"].copy()

st.title("🛍️ 소매업종 전문 분석")
st.caption("서울시 상가(상권)정보 중 '소매' 대분류 업종만 추출하여 심층 분석합니다.")

# ────────────────────────────────────────────────────────────
# 1. 전체 개요 KPI
# ────────────────────────────────────────────────────────────
st.header("📌 소매업종 전체 개요")

total_all = len(df_all)
total_retail = len(retail)
share = total_retail / total_all * 100 if total_all else 0
mid_n = retail["상권업종중분류명"].nunique()
top_mid_all = retail["상권업종중분류명"].value_counts().idxmax()

k1, k2, k3, k4 = st.columns(4)
k1.metric("서울 전체 소매업소 수", f"{total_retail:,}개")
k2.metric("전체 업종 대비 비중", f"{share:.1f}%")
k3.metric("소매 중분류 종류", f"{mid_n}개")
k4.metric("서울 최다 소매 중분류", top_mid_all)

st.divider()

# ────────────────────────────────────────────────────────────
# 2. 구별 소매업 현황 비교 (25개 구 전체)
# ────────────────────────────────────────────────────────────
st.header("🏙️ 구별 소매업 현황 비교")

gu_retail = retail.groupby("시군구명").size().reset_index(name="소매업소수")
gu_total = df_all.groupby("시군구명").size().reset_index(name="전체업소수")
gu_stat = gu_retail.merge(gu_total, on="시군구명")
gu_stat["소매업비중(%)"] = (gu_stat["소매업소수"] / gu_stat["전체업소수"] * 100).round(1)
gu_stat["구분"] = gu_stat["시군구명"].apply(lambda x: "관심 4개 구" if x in TOP4_GU else "기타")

c1, c2 = st.columns(2)
with c1:
    fig1 = px.bar(
        gu_stat.sort_values("소매업소수", ascending=True),
        x="소매업소수", y="시군구명", orientation="h", text="소매업소수",
...
with c2:
    fig2 = px.bar(
        gu_stat.sort_values("소매업비중(%)", ascending=True),
        x="소매업비중(%)", y="시군구명", orientation="h", text="소매업비중(%)",
        color="구분", color_discrete_map={"관심 4개 구": "#e74c3c", "기타": "#3498db"},
        title="구별 소매업 비중 (해당 구 전체 업종 대비 %)",
    )
...
# ────────────────────────────────────────────────────────────
# 3. 구별 소매업종 쏠림도 분석 (HHI 지수)
# ────────────────────────────────────────────────────────────
st.header("🧮 구별 소매업종 쏠림도 분석 (HHI 지수)")
st.caption(
    "HHI(허핀달-허쉬만 지수)는 구 내 소매 중분류별 점유율 제곱합입니다. "
    "값이 높을수록 특정 업종(예: 섬유·의복·신발)에 쏠려 있고, 낮을수록 다양한 업종이 고르게 분포합니다. "
    "(기준: 2,500 이상 = 쏠림/저경쟁, 1,500~2,500 = 보통, 1,500 미만 = 다양/과밀경쟁)"
)

hhi_rows = []
for gu, sub in retail.groupby("시군구명"):
...
# ────────────────────────────────────────────────────────────
# 4. 지역별 소매업종 심층 분석 (선택 + 직접 입력)
# ────────────────────────────────────────────────────────────
st.header("🔍 지역별 소매업종 심층 분석")

gu_list = sorted(retail["시군구명"].dropna().unique().tolist())
select_options = ["전체 (서울)"] + gu_list + ["✏️ 직접 입력"]

c1, c2 = st.columns([1, 1])
with c1:
    selected = st.selectbox("지역 선택 (구 단위)", select_options, index=0, key="retail_region_select")
with c2:
    manual_input = ""
    if selected == "✏️ 직접 입력":
        manual_input = st.text_input(
            ...
            key="retail_region_manual",
        )

if selected == "✏️ 직접 입력" and manual_input.strip():
    keyword = manual_input.strip()
    mask = (
        retail["시군구명"].str.contains(keyword, na=False)
        | retail["행정동명"].str.contains(keyword, na=False)
        | retail["법정동명"].str.contains(keyword, na=False)
    )
    fsub = retail[mask]
    region_label = keyword
elif selected == "✏️ 직접 입력":
    fsub = retail.iloc[0:0]
    region_label = "(지역명을 입력해주세요)"
elif selected == "전체 (서울)":
    fsub = retail
    region_label = "서울 전체"
else:
    fsub = retail[retail["시군구명"] == selected]
    region_label = selected

# 행정동 세부 필터 (선택 사항)
if not fsub.empty and selected not in ("전체 (서울)",):
    dong_list = sorted(fsub["행정동명"].dropna().unique().tolist())
    dong_select = st.multiselect("행정동으로 더 좁혀보기 (선택 사항)", dong_list, key="retail_dong_filter")
    if dong_select:
        fsub = fsub[fsub["행정동명"].isin(dong_select)]

st.subheader(f"'{region_label}' 소매업종 현황 ({len(fsub):,}개 업소)")
...
if fsub.empty:
    ...
    with b1:
        ...
        fig3 = px.bar(
            ...
            title="소매 중분류별 현황", color="개수", color_continuous_scale="Greens",
        )
    ...
    fig5 = px.bar(
        ...
        color_continuous_scale="Teal", title="소매 소분류 TOP 20",
    )
    ...
    fig6 = px.treemap(
        ...
        title="소매업종 트리맵 (박스가 클수록 업소 수가 많음)",
    ...
    fig7 = px.bar(
        ...
        color_continuous_scale="Purples", title="소매업 밀집 행정동 TOP 15",
    )
